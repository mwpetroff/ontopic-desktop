...
Be especially careful with "process": do NOT reuse the same broad, generic process phrase (e.g. a restatement of the meeting's own stated goal, like "document requirements" or "review the system") across many otherwise-unrelated supplier/input pairs just to fill the field — that isn't a real per-item connection, it's meeting framing. Only include "process" in a link when a *specific* processing step ties that particular supplier and input together (e.g. "invoices from Acme are keyed into the AP system"). If no such specific step is stated, leave "process" out of that link rather than padding it with something generic — a two-field supplier→input link is more useful and more honest than a five-field link where three of the fields are filler.

Be especially careful with "customer": it means whoever receives or benefits from the OUTPUT of the process being described — typically an internal team, department, or external end-customer. Do NOT label something "customer" just because it appears in the same sentence as an onboarding/document-collection process — check whether that same name or organization is already identified as a supplier (or described as a prospective/lost supplier) anywhere in the transcript or the lists above. If it is, it is playing a supplier role in this conversation, not a customer role, even in a sentence about onboarding it — leave "customer" out of that link rather than assigning a confusing dual role to the same entity.
